...
def RandomizedSearch():
    ...
    file0.flush()
    return parameters
def ElasticnetReg(parameters, X_train,y_train,X_test, y_test):
    model = ElasticNet(alpha=parameters[0],
                       l1_ratio=parameters[1],
                       fit_intercept=parameters[2],
                       precompute=parameters[3],
                       max_iter=parameters[4], 
                       tol=parameters[5], 
                       warm_start = parameters[6],
                       random_state = parameters[7],
                       selection=parameters[8])
...
while True:
    ...
    if test > 0.99:
        ...
        file1.write("Solution is not found!")
        file1.flush()
    else:
        k+=1
        continue
