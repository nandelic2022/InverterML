...
dfx1 = df[['i_a_k',
            ...
            'd_c_k-3']]
dfy1 = df.pop("u_c_k-1")
X_train, X_test, y_train, y_test = train_test_split(dfx1,dfy1, test_size = 0.3,shuffle=True)
...
def MLPRegCV(parameters, X_train,y_train,X_test, y_test):
    model = MLPRegressor(hidden_layer_sizes=parameters[0],
                         activation= parameters[1],
                         solver= parameters[2],
                         alpha= parameters[3],
                         batch_size = parameters[4],
                         learning_rate = parameters[5],
                         max_iter = parameters[6],
                         tol = parameters[7],
                         n_iter_no_change = parameters[8],
                         verbose = True)
