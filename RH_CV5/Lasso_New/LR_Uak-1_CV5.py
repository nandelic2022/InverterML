...
def RandomizedSearch():
    ...
    warmStart = random.choice([True, False])
    ...
    file0.flush()
    return parameters
def LASSOReg(parameters, X_train,y_train,X_test, y_test):
    model = Lasso(alpha=parameters[0],
                             fit_intercept=parameters[1], 
                             max_iter=parameters[2], 
                             tol=parameters[3], 
                             warm_start=parameters[4],  
                             random_state=parameters[5], 
                             selection=parameters[6])
...
while True:
    ...
    test = LASSOReg(HRParam,X_train,y_train, X_test, y_test)
    k+=1
    if test > 0.99:
